...
static constexpr double MIN_SPACE_RATIO_DEFAULT = 1.0 / 100,
                        MAX_SPACE_RATIO_DEFAULT = 1.0 / 10;

static void printArgumentErrorMessage(const char *func_name,
                                      const char *correct_argument_condition) {
  fprintf(stderr, "%s: argument needs to be %s.\n", func_name,
          correct_argument_condition);
}
...
void ACOSteiner::setMinDistance(double d) {
  ...
  printArgumentErrorMessage(
      __PRETTY_FUNCTION__, "no less than 0 and no more than half max_distance");
}
...
void ACOSteiner::setMaxDistance(double d) {
  ...
  printArgumentErrorMessage(__PRETTY_FUNCTION__,
                            "more than 0 and no less than twice min_distance");
}

void ACOSteiner::setMutationProbability(double value) {
  if (value >= 0 && value <= 1) {
    this->mutation_probability = value;
    return;
  }
...
void ACOSteiner::setEvaporationCofficient(double value) {
  if (value >= 0 && value <= 1) {
    this->evaporation_cofficient = value;
    return;
  }
  printArgumentErrorMessage(__PRETTY_FUNCTION__, "value from 0.0 to 1.0");
}
...
void ACOSteiner::countTime() {
  for (const Ant *a : *(this->table))
    a->evaporate(this->evaporation_cofficient);
}
