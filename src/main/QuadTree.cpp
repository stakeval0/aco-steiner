...
static int mortonDepth(uint morton1, uint morton2) {
  ...
  int i;
  for (i = sizeof(morton1) * 8 - 2; i >= 0 && !((xclusive_or >> i) & 0x3);
       i -= 2)
    ;
...
static uint moveToCenterInParentMorton(uint morton, uint fixed_depth,
                                       uint mask_xy) {
  ...
  return (morton & ~(mask_xy >> fixed_depth * 2)) ^
         (direct_mask & mask_xy >> 2 * fixed_depth);
}

static void pushNewMortonArea(stack<array<uint, 3>> &s, uint left_up_morton,
                              uint right_bottom_morton) {
  array<uint, 3> tmp;
  tmp[0] = left_up_morton;
  tmp[1] = right_bottom_morton;
...
/*
  NOTE:
  QuadTreeNodeの等価演算子を、座標の等価をポインタとして判断する案があったが、
...
template <class T>
QuadTree<T>::QuadTree(const array<double, 2> &offset,
...
template <class T>
QuadTree<T>::QuadTree(const array<double, 2> &offset,
                      const array<double, 2> &size, int level) {
  if (level > sizeof(ushort) * 8) {
    fprintf(stderr, "%s: level is too big.\n", __PRETTY_FUNCTION__);
    exit(EXIT_FAILURE);
  }
...
template <class T>
vector<QuadTreeNode<T>> QuadTree<T>::reachablePoints(double cx, double cy,
                                                     double width,
                                                     double height) const {
...
vector<QuadTreeNode<T>> QuadTree<T>::reachablePoints(
  ...
  double p1_x = max(cx - hw, 0.), p1_y = max(cy - hh, 0.),
         p2_x = min(cx + hw, this->width()),
         p2_y = min(cy + hh, this->height());
  const int MAX_DEPTH =
      ceil(log2(this->width() * this->height() / (width * height)) / 2);
  array<uint, 3> tmp;
  tmp[0] = mortonNumber(p1_x, p1_y);
  tmp[1] = mortonNumber(p2_x, p2_y);
  tmp[2] = mortonDepth(tmp[0], tmp[1]);
  s.push(tmp);
  while (!s.empty()) {
    tmp = s.top();
    ...
    array<uint, 4> current_mortones = {tmp[0], p2_masked_x ^ p1_masked_y,
                                       p1_masked_x ^ p2_masked_y, tmp[1]};
    if (mortonInDepth(current_mortones[0], next_depth) ==
        mortonInDepth(current_mortones[1], next_depth)) {
      // この時探索領域は縦長のように思える
      pushNewMortonArea(
          s,
          ...
          current_mortones[3]);
      pushNewMortonArea(
          s, current_mortones[0],
          moveToCenterInParentMorton(current_mortones[1], next_depth, MASK_Y));
    } else if (mortonInDepth(current_mortones[0], next_depth) ==
               mortonInDepth(current_mortones[2], next_depth)) {
      // この時探索領域は横長のように思える
      pushNewMortonArea(
          s,
      ...
      pushNewMortonArea(s,
                        moveToCenterInParentMorton(current_mortones[3],
                                                   next_depth, __UINT32_MAX__),
                        current_mortones[3]);
      pushNewMortonArea(
          s,
          moveToCenterInParentMorton(current_mortones[2], next_depth, MASK_Y),
      ...
      pushNewMortonArea(
          ...
          moveToCenterInParentMorton(current_mortones[1], next_depth, MASK_X),
          moveToCenterInParentMorton(current_mortones[1], next_depth, MASK_Y));
      pushNewMortonArea(s, current_mortones[0],
                        moveToCenterInParentMorton(current_mortones[0],
                                                   next_depth, __UINT32_MAX__));
    }
  }
  return ret;
...
bool QuadTree<T>::inWorld(double x, double y) const {
  ...
  if (x_in_range < 0 || y_in_range < 0 || x_in_range > this->width() ||
      y_in_range > this->height()) {
...
uint QuadTree<T>::mortonNumber(double x, double y) const {
  ...
  ushort i = min((int)((x - this->offset_v[0]) / this->unit_width),
                 (1 << this->level) - 1);
  ushort j = min((int)((y - this->offset_v[1]) / this->unit_height),
                 (1 << this->level) - 1);
...
template <class T>
void QuadTree<T>::searchMorton(
    uint morton, int search_depth,
  ...
  const uint
      LOWER_BIT_MAX =
          __UINT32_MAX__ >>
          (2 *
           search_depth),  // ここをbitsetに付いてきたUINT32_MAXにして、3項演算子でない状態でやっていたら何故か値がsearch_depth==16でUINT32_MAXになった
      MORTON_HIGHER_BIT = morton & ~LOWER_BIT_MAX,
      MORTON_MAX = MORTON_HIGHER_BIT + LOWER_BIT_MAX;
  ...
  for (; e != this->quad_tree.end() && e->first <= MORTON_MAX; e++) {
    ...
    for (
        const auto &g :
        e->second) {  // NOTE:
                      // 本当はsetをbufに詰められる様にした方が速そうだが、簡単なのでこうする
